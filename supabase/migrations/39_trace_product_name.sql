-- Migration 39: include product name in sale_chain
-- ============================================================
-- Each sale in the trace's sale_chain now also reports the
-- product_name from the lot it was drawn from, so the trace
-- page can show "Wool" / "Yarn" / etc. alongside the volume.
--
-- Same function shape as migration 38 — only the sale_path CTE
-- and the sale_chain JSON aggregation changed.
...
begin
  ...
  sale_path as (
    -- Anchor: the requested sale at depth 0. Pull product_name
    -- from the inventory lot the sale draws from.
    select
      s.id,
      s.code,
