-- Migration 38: get_trace_by_sale_code returns the full sale chain
-- ============================================================
-- Adds a `sale_chain` array to the trace JSON, walking back
-- through every sale that contributed to the requested sale's
-- material. Each entry has sale_code, date, volume, seller, buyer.
-- Ordered earliest first (so the chain reads top-to-bottom on
-- the trace page).
--
-- The existing `chain` recursive CTE (used for landbase volume
-- attribution + purchasing_org) is unchanged. We add a parallel
-- `sale_path` recursive CTE that walks sales explicitly.
...
begin
  ...
  with recursive
  -- Walk back through purchases to source landbases for volume
  -- attribution. Unchanged from migration 37.
  chain as (
    select
      p.id              as purchase_id,
  ...
  -- New: walk back through sales themselves (the hops the
  -- material took on its way to the requested sale).
  sale_path as (
    -- Anchor: the requested sale at depth 0
    select
      s.id,
      s.code,
    ...
    -- Recurse: for each sale in the path, find sales that
    -- contributed to its lot's batch via "received" purchases.
    select
      src_sale.id,
      src_sale.code,
      src_sale.organization_id,
      src_sale.buyer_org_id,
      src_sale.buyer_name,
      src_sale.volume,
      src_sale.volume_unit,
      src_sale.sale_date,
      src_sale.inventory_lot_id,
      sp.depth + 1
    from sale_path sp
    ...
    where bi.source_type = 'raw_purchase'
      ...
      and sp.depth < 50  -- safety cap against infinite recursion
  )
  select jsonb_build_object(
    ...
    -- Earliest sale first (depth desc), final sale last
    'sale_chain', coalesce((
      select jsonb_agg(jsonb_build_object(
        'sale_code',   d.code,
      ...
      from (
        -- Dedupe by sale id (in case the same sale was visited
        -- through multiple branches of the chain).
        select distinct on (id)
          id, code, sale_date, volume, volume_unit,
